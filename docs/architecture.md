...
存档以受限文件根目录读取，边复制边校验大小与 SHA-256，形成 Unix open-unlink 临时文件快照。验证与发送使用同一文件描述符，避免原路径在校验后变化影响出站。CRLF 预检完成后才建立 SMTP 连接；正文通过 32 KiB 缓冲流式点转义。进程共享资源预算在领取租约/配额前实施背压，详见 [5B 资源与验收边界](phase-5b-resources.md)。
